function [DimReductionIm]= horse_DimReductionPCA(code_layer_transfer,TestIm,num,flag)
% feature dimension reduction using PCA.
% input: code: code layer features
%        num: number of the Dim you want to reduce to. 1 or 3.
...
TransferIm_new = (reshape(TransferIm,size(NewChannels,1),size(NewChannels,2),...
     size(NewChannels,3)));
